...
class VendorOrder {
  ...
  factory VendorOrder.fromJson(Map<String, dynamic> json) {
    ...
    final riderBreakdown = mainOrder['riderPayoutBreakdown'] is Map
        ? Map<String, dynamic>.from(mainOrder['riderPayoutBreakdown'] as Map)
        : <String, dynamic>{};
    final subtotal = double.tryParse('${json['subtotal'] ?? 0}') ?? 0.0;
    final platformFee = double.tryParse('${json['platformFee'] ?? 0}') ?? 0.0;
    ...
    return VendorOrder(
      ...
      riderPayout:
          ...
          0.0,
      riderDistanceKm:
          double.tryParse('${riderBreakdown['totalDistanceKm'] ?? 0}') ?? 0.0,
      riderRatePerKm:
          double.tryParse('${riderBreakdown['ratePerKm'] ?? 0}') ?? 0.0,
    );
  }

  VendorOrder copyWith({String? orderStatus, String? rejectionReason}) {
    return VendorOrder(
      id: id,
      userId: userId,
      buyerName: buyerName,
      ...
      riderDistanceKm: riderDistanceKm,
      riderRatePerKm: riderRatePerKm,
    );
  }
}
...
class OrdersService {
  ...
  Future<VendorStats> fetchVendorStats() async {
    ...
    if (resp.statusCode == 200) {
      ...
      final message =
          _safeMessage(resp.body) ??
          'Failed to load vendor stats (HTTP ${resp.statusCode}).';
  ...
  Future<List<VendorOrder>> fetchOrders() async {
    ...
    if (resp.statusCode == 200) {
      ...
      final message =
          _safeMessage(resp.body) ??
          'Failed to load orders (HTTP ${resp.statusCode}).';
      throw Exception(message);
    }
  }
  ...
  Future<void> updateOrderStatus(String shipmentId, String newStatus) async {
    if (newStatus == 'out_for_delivery' || newStatus == 'delivered') {
      throw Exception(
        'Rider pickup and delivery statuses are updated through OTP verification.',
      );
    }
    ...
    if (resp.statusCode != 200) {
      final message =
          _safeMessage(resp.body) ?? 'Failed to update order status.';
      throw Exception(message);
    }
  }
...
class _OrderTile extends StatelessWidget {
  ...
  final Future<void> Function(VendorOrder order, String newStatus)
  onUpdateStatus;
        ...
        children: [
          ...
          if (order.pickupCode.trim().isNotEmpty) ...[
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: VendorUi.blue.withValues(alpha: 0.08),
                borderRadius: BorderRadius.circular(16),
                border: Border.all(
                  color: VendorUi.blue.withValues(alpha: 0.24),
                ),
              ),
              child: Row(
                children: [
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                Text(
                  'Vendor payout: ₦${order.vendorPayout.toStringAsFixed(2)} • Platform fee: ₦${order.platformFee.toStringAsFixed(2)}',
                  style: const TextStyle(color: VendorUi.textMuted),
                ),
                const SizedBox(height: 4),
                Text(
                  order.riderRatePerKm > 0
                      ? 'Rider payout: ₦${order.riderPayout.toStringAsFixed(2)} • ${order.riderDistanceKm.toStringAsFixed(2)} km × ₦${order.riderRatePerKm.toStringAsFixed(2)}/km'
                      : 'Rider payout: ₦${order.riderPayout.toStringAsFixed(2)} • Delivery fee ₦${order.deliveryFee.toStringAsFixed(2)}',
                  style: const TextStyle(color: VendorUi.textMuted),
                ),
              ],
            ),
            ...
            children: [
              Column(
                ...
                children: [
                  const Text(
                    'Order total',
                    style: TextStyle(color: VendorUi.textMuted, fontSize: 12),
                  ),
                  const SizedBox(height: 4),
                  Text(
      ...
      children: [
        ...
        Column(
          ...
          children: [
            ...
            const Text(
              'Total',
              style: TextStyle(color: VendorUi.textMuted, fontSize: 10),
            ),
          ],
        ),
      ],
